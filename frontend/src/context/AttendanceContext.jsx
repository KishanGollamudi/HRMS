...
export const AttendanceProvider = ({ children }) => {
  ...
  const buildEntries = (sprintId) => {
    ...
    const matched = appEmployees.filter((emp) =>
      pairs.some(
        (pair) =>
          pair.technology?.toLowerCase() === emp.technology?.toLowerCase() &&
          pair.cohort?.toLowerCase() === emp.cohort?.toLowerCase(),
      ),
    );
  ...
  const ensureSession = async (sprintId, date) => {
    const key = getSessionKey(sprintId, date);
    ...
    const freshEntries = buildEntries(sprintId);
    setSessions((prev) => ({
      ...prev,
