...
const COHORT_COLORS = ["#0d9488","#f59e0b","#8b5cf6","#3b82f6","#ec4899","#059669"];
const cohortColor = (cohort, idx) => COHORT_COLORS[idx % COHORT_COLORS.length];
...
function SprintAccordion({ sprintTitle, sprintMeta, cohortMap, defaultOpen }) {
  ...
  return (
    <div style={{ background: T.card, border: `1.5px solid ${T.border}`, borderRadius: 16, overflow: "hidden", boxShadow: T.shadow }}>
      ...
      <button
        ...
        style={{
          ...
          padding: "14px 20px", background: T.bg, border: "none", cursor: "pointer",
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          ...
          <span style={{ color: T.accent, fontSize: 14, fontWeight: 700 }}>{sprintTitle}</span>
          {sprintMeta?.status && (
            <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 20,
              background: sprintMeta.status === "Completed" ? "rgba(5,150,105,0.1)" : T.accentBg,
          ...
          {cohortKeys.map((cohort, idx) => {
            ...
            const cPct  = cRecs.length ? Math.round((cPres / cRecs.length) * 100) : 0;
            return (
              <span key={cohort} style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
        ...
        {open && (
          ...
          >
            {cohortKeys.map((cohort, ci) => {
              const color = cohortColor(cohort, ci);
              const cRecs = cohortMap[cohort];
              return (
                <div key={cohort}>
                  {/* Cohort sub-header */}
                  <div style={{
                    ...
                    <span style={{ color, fontSize: 12, fontWeight: 700 }}>{cohort}</span>
                    <span style={{ color: T.muted, fontSize: 11 }}>— {cRecs.length} records</span>
                    <span style={{ marginLeft: "auto", fontSize: 11, fontWeight: 600 }}>
                      <span style={{ color: "#059669" }}>{cRecs.filter((r) => r.status === "Present").length} present</span>
                      {" · "}
                      <span style={{ color: T.amber }}>{cRecs.filter((r) => r.status === "Late").length} late</span>
                      {" · "}
                      <span style={{ color: T.red }}>{cRecs.filter((r) => r.status === "Absent").length} absent</span>
                    </span>
                  </div>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                    <thead>
                      <tr style={{ background: T.bg, borderBottom: `1px solid ${T.border}` }}>
                        {["Emp ID", "Name", "Technology", "Date", "Status"].map((h) => (
                          <th key={h} style={{ textAlign: "left", padding: "8px 16px", color: T.muted, fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {cRecs.map((emp) => {
                        const ss = STACK_STYLES[emp.techStack] || { color: T.muted, bg: T.accentBg };
                        const st = STATUS_STYLES[emp.status]   || STATUS_STYLES.Absent;
                        return (
                          <tr key={`${emp.id}-${emp.date}`}
                            style={{ borderBottom: `1px solid ${T.border}`, transition: "background 0.1s" }}
                            onMouseEnter={(e) => e.currentTarget.style.background = T.bg}
                            onMouseLeave={(e) => e.currentTarget.style.background = ""}>
                            <td style={{ padding: "9px 16px", fontFamily: "monospace", fontSize: 11, color: T.muted }}>{emp.id}</td>
                            <td style={{ padding: "9px 16px", color: T.text, fontWeight: 600 }}>{emp.name}</td>
                            <td style={{ padding: "9px 16px" }}>
                              <span style={{ background: ss.bg, color: ss.color, fontSize: 11, fontWeight: 700, padding: "2px 9px", borderRadius: 20 }}>{emp.techStack}</span>
                            </td>
                            <td style={{ padding: "9px 16px", color: T.muted, fontSize: 12 }}>{emp.date}</td>
                            <td style={{ padding: "9px 16px" }}>
                              <span style={{ background: st.bg, color: st.color, fontSize: 11, fontWeight: 700, padding: "2px 9px", borderRadius: 20 }}>{emp.status}</span>
                            </td>
                          </tr>
                        );
                      })}
...
export default function AttendanceList() {
    ...
    Promise.all(sprints.map((s) =>
      ...
      responses.forEach((res, i) => {
        ...
        list.forEach((r) => {
          ...
          const dateStr = d == null ? "" : typeof d === "string" ? d.slice(0, 10) : String(d).slice(0, 10);
          out.push({
            id:        r.empId,
            name:      r.employeeName,
  ...
  const TECH_STACKS = ["All", "Java", "Python", "Devops", "DotNet", "SalesForce"];
  ...
  const allRecords = useMemo(() => {
    ...
    Object.entries(attendance).forEach(([date, dayRecords]) => {
      dayRecords.forEach((r) => {
        ...
        records.push({ id: r.empId, name: r.name, techStack: r.technology, cohort: r.cohort, sprint: r.sprint, sprintId: sprint?.id ?? r.sprint, date, status: r.status });
      });
    });
    return records;
  ...
  const filtered = useMemo(() => {
    ...
    return allRecords.filter((e) => {
      const matchSearch  = (e.name ?? "").toLowerCase().includes(q) || String(e.id ?? "").toLowerCase().includes(q);
      ...
      return matchSearch && matchStack && matchCohort && matchDateFrom && matchDateTo;
    });
  ...
  const hasFilters = cohortFilter !== "All" || stackFilter !== "All" || search || dateFrom || dateTo;

  const handleDownload = () => {
    const header = "ID,Name,Tech Stack,Sprint,Cohort,Date,Status";
    const rows   = filtered.map((e) => `${e.id},${e.name},${e.techStack},${e.sprint},${e.cohort},${e.date},${e.status}`);
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <h1 style={{ color: T.text, fontSize: 22, fontWeight: 800, margin: 0 }}>Attendance Overview</h1>
              <p style={{ color: T.muted, fontSize: 12, margin: 0 }}>
                {Object.keys(grouped).length} sprint{Object.keys(grouped).length !== 1 ? "s" : ""} · {filtered.length} records
              </p>
            </div>
          </div>
          <button onClick={handleDownload}
            style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 18px", borderRadius: 12, background: T.accent, color: "#fff", fontWeight: 700, fontSize: 13, border: "none", cursor: "pointer", boxShadow: T.accentGlow }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
          ...
          {/* Cohort filter */}
          <select value={cohortFilter} onChange={(e) => setCohortFilter(e.target.value)} style={{ ...inp, height: 38, cursor: "pointer" }}>
            {cohorts.map((o) => <option key={o} value={o}>{o === "All" ? "All Cohorts" : o}</option>)}
          </select>

          {/* Tech stack filter */}
          <select value={stackFilter} onChange={(e) => setStackFilter(e.target.value)} style={{ ...inp, height: 38, cursor: "pointer" }}>
            {TECH_STACKS.map((o) => <option key={o} value={o}>{o === "All" ? "All Tech Stacks" : o}</option>)}
          </select>
          ...
          {/* Clear */}
          {hasFilters && (
            <button onClick={() => { setCohortFilter("All"); setStackFilter("All"); setSearch(""); setDateFrom(""); setDateTo(""); }}
              style={{ background: "none", border: "none", color: T.accent, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
              Clear ×
            </button>
          )}
        </div>
        ...
        {!USE_MOCK && loadingApi ? (
          ...
          <div style={{ textAlign: "center", padding: "60px 0", color: T.muted }}>
            ...
            <p style={{ fontSize: 12, marginTop: 4 }}>Try adjusting your filters, or mark attendance on a sprint date.</p>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {Object.entries(grouped).map(([sprintTitle, cohortMap], idx) => {
        ...
        <p style={{ color: T.muted, fontSize: 11 }}>
          ...
          {dateFrom && <span> · from <span style={{ color: T.accent }}>{dateFrom}</span></span>}
          {dateTo   && <span> · to <span style={{ color: T.accent }}>{dateTo}</span></span>}
        </p>
      </div>
    </motion.div>
  );
